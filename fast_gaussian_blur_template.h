...
void fast_gaussian_blur(T *&in, T *&out, const int w, const int h, const int c, const float sigma, const unsigned int n)
{
    ...
    {
    case 1:
        fast_gaussian_blur<T, 1>(in, out, w, h, c, sigma);
        break;
    case 2:
        fast_gaussian_blur<T, 2>(in, out, w, h, c, sigma);
        break;
    case 3:
        fast_gaussian_blur<T>(in, out, w, h, c, sigma);
        break; // specialized 3 passes version
    case 4:
        fast_gaussian_blur<T, 4>(in, out, w, h, c, sigma);
        break;
    case 5:
        fast_gaussian_blur<T, 5>(in, out, w, h, c, sigma);
        break;
    case 6:
        fast_gaussian_blur<T, 6>(in, out, w, h, c, sigma);
        break;
    case 7:
        fast_gaussian_blur<T, 7>(in, out, w, h, c, sigma);
        break;
    case 8:
        fast_gaussian_blur<T, 8>(in, out, w, h, c, sigma);
        break;
    case 9:
        fast_gaussian_blur<T, 9>(in, out, w, h, c, sigma);
        break;
    case 10:
        fast_gaussian_blur<T, 10>(in, out, w, h, c, sigma);
        break;
    default:
        printf("fast_gaussian_blur with %d passes is not supported yet. Add a specific case if possible or fall back to the generic version.\n", n);
        break;
