...
#x1 = np.insert(x1,0,np.zeros(b))
#x1 = np.insert(x1, x1.size, np.zeros(b-1))
...
output1_debug = np.zeros(x1.size + r*4, dtype=int)
output2_debug = np.zeros(x1.size + r*4, dtype=int)
...
for i in range(b):
    ...
    sum1= sum1 + x1[e]
    if (i >= r):
        output1_debug[i - r] = sum1
        d.append(sum1)
        sum2 = sum2 - d[0] + d[b]
        if(i >= r*2):
            output2_debug[i - r*2] = sum2

for i in range(1, x1.size ):
    e = (((x1.size-1)- abs(b-i) % (x1.size-1)),abs(b-i))[abs(b-i) < (x1.size)]
    sum1 = sum1 - x1[e] + x1[i]
    #print(sum1, "+",x1[i], "-",x1[abs(b - i)], i, abs(b - i))
    #print(e,i)
    d.append(sum1)
    sum2 = sum2 - d[0] + d[b]
    output1_debug[i + r] = sum1
...
for i in range(x1.size , x1.size + r*2):
    sum1 = sum1 + x1[(x1.size-1-i) % (x1.size-1)] - x1[abs(b - i)]
    #print(abs(b-i),i-b, (x1.size-1-i) % (x1.size-1) )
    #print(sum1, "+", x1[(x1.size-1-i) % (x1.size-1)], "-", x1[i-b],(x1.size-1-i) % (x1.size-1),i-b)
    d.append(sum1)
    sum2 = sum2 - d[0] + d[b]
    output1_debug[i + r] = sum1
    output2_debug[i] = sum2
...
j = 1
for i in range(x1.size + r*2, x1.size + r*3 ):
    sum1 = sum1 - x1[x1.size - j ]
    #print(x1.size-j)
    #(ri < end ? ri : max_end - ri % max_end
    d.append(sum1)
    sum2 = sum2 - d[0] + d[b]
    output1_debug[i + r] = sum1
    output2_debug[i] = sum2
...
j = 1
for i in range(x1.size + r*3, x1.size + r*4 ):
    sum2 = sum2 - d[j]
    output2_debug[i] = sum2
    j+=1
